...
function generateWeightData(startWeight: number, targetWeight: number, weeks: number = 39) {
...
function createSampleNutritionPlan(clientId: string, coachId: string) {
  return {
    clientId,
...
async function seedClientData() {
  try {
    ...
    // Get all clients
    const clientsSnapshot = await getDocs(collection(db, 'client_profiles'));
    console.log(`Found ${clientsSnapshot.size} clients`);
    
    for (const clientDoc of clientsSnapshot.docs) {
      const clientId = clientDoc.id;
      const clientData = clientDoc.data();
      console.log(`\nSeeding data for client: ${clientData.fullName} (${clientId})`);
      ...
      await setDoc(doc(db, 'client_progress', clientId), progressData);
      ...
      await setDoc(doc(db, 'nutrition_plans', clientId), nutritionPlan);
